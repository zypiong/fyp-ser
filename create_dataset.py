...
from config import TESS_ORIGINAL_PATH, RAVDESS_ORIGINAL_PATH
...
def create_tess_folders(path):
    ...
    actor_path25 = DATASET_DIR_PATH + "Actor_25"
    actor_path26 = DATASET_DIR_PATH + "Actor_26"
    if not os.path.exists(actor_path25):
        os.makedirs(actor_path25)
    if not os.path.exists(actor_path26):
        os.makedirs(actor_path26)
    ...
    for subdir, _, filenames in os.walk(path):
        ...
        for file in filenames:
            if file.startswith('YAF'):
                destination_path = DATASET_DIR_PATH + 'Actor_25/'
            else:
                destination_path = DATASET_DIR_PATH + 'Actor_26/'
...
def merge_ravdess(path):
    for i in range(1, 25):
        actor_path = DATASET_DIR_PATH + "Actor_" + f"{i:02d}"
    ...
    for subdir, _, filenames in os.walk(path):
        for file in filenames:
            ...
            path = subdir.split("/")
            new_path = DATASET_DIR_PATH + path[-1]
            if not os.path.exists(new_path):
                shutil.copy(old_path, new_path)


if __name__ == '__main__':
    # merge_ravdess(RAVDESS_ORIGINAL_PATH)
    create_tess_folders(TESS_ORIGINAL_PATH)
